...
func (c *NodesCollector) Collect(ctx context.Context, reg *cratedb.Registry, st *store.Store) error {
	...
	for _, row := range resp.Rows {
		info := cratedb.NodeInfo{
			...
			CPUPercent: int16(cratedb.ToFloat64(row[4])),
			CPUSystem:  int16(cratedb.ToFloat64(row[5])),
			CPUUser:    int16(cratedb.ToFloat64(row[6])),
			HeapUsed:   int64(cratedb.ToFloat64(row[7])),
			HeapMax:    int64(cratedb.ToFloat64(row[8])),
			HeapFree:   int64(cratedb.ToFloat64(row[9])),
			FSTotal:    int64(cratedb.ToFloat64(row[10])),
			FSUsed:     int64(cratedb.ToFloat64(row[11])),
			FSAvail:    int64(cratedb.ToFloat64(row[12])),
			MemUsed:    int64(cratedb.ToFloat64(row[13])),
			MemFree:    int64(cratedb.ToFloat64(row[14])),
			MemTotal:   int64(cratedb.ToFloat64(row[15])),
			Load:       [3]float64{cratedb.ToFloat64(row[16]), cratedb.ToFloat64(row[17]), cratedb.ToFloat64(row[18])},
			Version:        cratedb.ToString(row[19]),
			FSReads:        int64(cratedb.ToFloat64(row[20])),
			FSWrites:       int64(cratedb.ToFloat64(row[21])),
			FSBytesRead:    int64(cratedb.ToFloat64(row[22])),
			FSBytesWritten: int64(cratedb.ToFloat64(row[23])),
...
func parseThreadPools(v interface{}) []cratedb.ThreadPoolStats {
	...
	for _, item := range arr {
		...
		pools = append(pools, cratedb.ThreadPoolStats{
			...
			Active:    int64(cratedb.ToFloat64(m["active"])),
			Queue:     int64(cratedb.ToFloat64(m["queue"])),
			Rejected:  int64(cratedb.ToFloat64(m["rejected"])),
			Completed: int64(cratedb.ToFloat64(m["completed"])),
			Threads:   int64(cratedb.ToFloat64(m["threads"])),
			Largest:   int64(cratedb.ToFloat64(m["largest"])),
		})
	}
	return pools
}
